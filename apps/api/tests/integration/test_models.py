"""Integration tests for model persistence with PostgreSQL."""
...
class TestBankSessionIntegration:
    ...
    def test_create_and_retrieve_bank_session(self, postgres_session: Session) -> None:
        ...
        retrieved = postgres_session.get(BankSession, session.id)
        assert retrieved is not None
        assert retrieved.session_id == "integration-test-session"
        assert retrieved.bank_key == "test-bank-001"
        assert retrieved.created_at is not None

    def test_bank_session_update(self, postgres_session: Session) -> None:
...
class TestCategoryIntegration:
    ...
    def test_create_category_with_parent(self, postgres_session: Session) -> None:
        """Test Category parent-child relationship."""
        parent = Category(name="Parent Category")
        postgres_session.add(parent)
        postgres_session.flush()

        child = Category(name="Child Category", parent_id=parent.id)
        postgres_session.add(child)
        postgres_session.flush()

        # Verify relationship
        retrieved_child = postgres_session.get(Category, child.id)
        assert retrieved_child.parent_id == parent.id
        assert retrieved_child.parent.name == "Parent Category"

    def test_category_closure_entry(self, postgres_session: Session) -> None:
        ...
        retrieved = (
            postgres_session.query(CategoryClosure)
...
class TestTransactionIntegration:
    ...
    def test_create_transaction(self, postgres_session: Session) -> None:
        ...
        # Verify retrieval
        retrieved = postgres_session.get(Transaction, transaction.id)
        assert retrieved is not None
        assert retrieved.description == "Test transaction"
        assert retrieved.amount == Decimal("123.45")

    def test_transaction_decimal_precision(self, postgres_session: Session) -> None:
...
class TestOnlinePurchaseIntegration:
    ...
    def test_create_online_purchase(self, postgres_session: Session) -> None:
        ...
        postgres_session.add(purchase)
        postgres_session.flush()

        assert purchase.id is not None

        # Verify defaults from database
        retrieved = postgres_session.get(OnlinePurchase, purchase.id)
    ...
    def test_online_purchase_defaults(self, postgres_session: Session) -> None:
        ...
        postgres_session.refresh(purchase)
...
class TestTransactionCategoryIntegration:
    ...
    def test_link_transaction_to_category(self, postgres_session: Session) -> None:
        ...
        postgres_session.add_all([transaction, category])
        postgres_session.flush()
        ...
        # Verify link
        retrieved = (
            postgres_session.query(TransactionCategory)
    ...
    def test_transaction_category_relationships(
        ...
        retrieved = postgres_session.get(TransactionCategory, link.id)
