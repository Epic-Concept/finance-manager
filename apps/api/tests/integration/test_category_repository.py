"""Integration tests for CategoryRepository with PostgreSQL."""
...
class TestCategoryRepositoryIntegration:
    """Integration tests for CategoryRepository against PostgreSQL."""

    def test_create_root_category(self, postgres_session: Session) -> None:
        """Test creating a root category creates self-referential closure entry."""
        repo = CategoryRepository(postgres_session)
    ...
    def test_get_descendants(self, postgres_session: Session) -> None:
    ...
    def test_move_category_updates_closure(self, postgres_session: Session) -> None:
    ...
    def test_delete_category_with_cascade(self, postgres_session: Session) -> None:
        ...
        assert postgres_session.get(Category, child_id) is None
        assert postgres_session.get(Category, grandchild_id) is None

        # Root should still exist
        assert postgres_session.get(Category, root.id) is not None
    ...
    def test_subtree_transaction_sum(self, postgres_session: Session) -> None:
        ...
        postgres_session.add_all([txn1, txn2, txn3])
        postgres_session.flush()
        ...
        postgres_session.add_all([link1, link2, link3])
        postgres_session.flush()
    ...
    def test_deeply_nested_hierarchy(self, postgres_session: Session) -> None:
