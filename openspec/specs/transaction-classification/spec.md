...
Defines the gather/decide engine: the collection loop, the deterministic evidence policy (tiers, required-tier table, combination rules, the itemized invariant), and how a transaction resolves to a categorization (splits summing to total) with confidence and an evidence chain.
