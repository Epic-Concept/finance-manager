...
Defines the first-class `Evidence` type and the gatherer contract: the evidence claim/type/source/strength/itemized fields, the strength tier ordering, and how gatherers (rules, history, web lookup, receipt hunt, LLM inference) emit evidence without deciding.
