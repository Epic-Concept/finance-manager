...
Defines the core database schema for financial transaction management.
...
Categories MUST support hierarchical organization using the closure table pattern.

#### Scenario: Category table structure
- Given the finance schema exists
- When the categories table is created
- Then it contains columns for id, name, parent_id, description, created_at, updated_at
